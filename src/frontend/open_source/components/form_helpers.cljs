(ns open-source.components.form-helpers
  ...
  (:require-macros [open-source.utils :refer [tv]]))
...
(defn field [type {:keys [data form-id tip dk attr-name required label] :as opts}]
  (let [errors (get-in @data [:errors attr-name])]
